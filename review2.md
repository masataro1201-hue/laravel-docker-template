...
### findメソッドで実行しているSQLは何か
  - select文を実行している。findメソッドの引数でwhere句の値を指定している。
### findメソッドで取得できる値は何か
  - モデルインスタンス。データベースの1レコードに対応するオブジェクトが取得できる
...
### traitとclassの違いとは
  - trait自体はインスタンス化することができない。
    - メリット、複数のクラス間でコードを共通化・再利用が可能
    - デメリット、インスタンス化できないため単体ではオブジェクトとして生成できない。
  - classはインスタンスが可能
    -メリット、複数のオブジェクトを同じクラスから作ることが可能 
    - デメリット、継承の制約がある。一つのクラスに対して継承は一つしかできない。
### traitを使用するメリットとは
...
### RequestクラスからFormRequestクラスに変更した理由
  - Requestクラスにはないバリデーション処理を実装するため。
